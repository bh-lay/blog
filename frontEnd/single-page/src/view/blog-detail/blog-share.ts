...
function loadImg (src: string): Promise<HTMLImageElement | undefined> {
...
const buildQRCodeImg = (url: string) => {
	...
	}).then((QRCodeUrl: string) => loadImg(QRCodeUrl))
}

const render = (coverImg: HTMLImageElement | undefined, footerImg: HTMLImageElement | undefined, QRCodeImg: HTMLImageElement | undefined) => {
	const img = new Image()
	...
	if (QRCodeImg) {
		const QRCodeWidth = 120 * pixelRatio
		// 将二维码绘制进 canvas
...
export function createShareCard ({ title, intro, url, coverUrl }: ShareCardOptions) {
	// 构建页脚图片
	const footerDataUrl = buildFooterImageDataUrl(title, intro)
	return Promise.all([
		// 加载封面图
		loadImg(coverUrl),
		// 加载页脚图片
